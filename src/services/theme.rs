...
use crate::models::{
    CreateThemeRequest, CssVariable, DropboxTemplate, DropboxTemplateType, SiteConfig,
    ThemeFilters, ThemeSettings, UpdateThemeRequest,
};
...
impl ThemeService {
    ...
    pub async fn create_theme(&self, request: CreateThemeRequest) -> Result<ThemeSettings> {
        ...
        if self
            .database
            .get_theme_by_name(&request.name)
            .await?
            .is_some()
        {
        ...
        info!(
            "Created theme: {} ({})",
            created_theme.display_name, created_theme.name
        );
        Ok(created_theme)
    }
            ...
            Ok(response) => {
                for entry in &response.entries {
                    ...
                    if entry.name.ends_with(".css") {
                        match self
                            .process_dropbox_css_file(&entry.name, &entry.path_lower)
                            .await
                        {
                ...
                for entry in &response.entries {
                    if entry.name.ends_with(".css") {
                            ...
                            .await
                        {
                            ...
                            Err(e) => {
                                warn!("Failed to process theme CSS file {}: {}", entry.name, e)
                            }
                        }
                    }
                }
    ...
    pub fn validate_css(&self, css: &str) -> Result<Vec<String>> {
        ...
        if css.contains("javascript:") {
            return Err(anyhow::anyhow!(
                "CSS contains potentially unsafe javascript: URLs"
            ));
        }
    ...
    pub async fn create_preset_themes(&self) -> Result<()> {
        ...
        let minimal_theme = CreateThemeRequest {
            ...
            custom_css: Some(
                ".article { line-height: 1.8; } h1, h2, h3 { font-weight: 300; }".to_string(),
            ),
            header_style: crate::models::HeaderStyle {
