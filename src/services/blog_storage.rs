...
impl RateLimiter {
    ...
    async fn check_rate_limit(&mut self) -> Result<()> {
        ...
        self.requests
            .retain(|&request_time| now.duration_since(request_time) < self.time_window);
...
impl BlogStorageService {
    ...
    pub async fn list_published_posts(&self) -> Result<Vec<BlogPost>> {
        ...
        let folder_result = self
            .dropbox_client
            .list_folder(&self.folders.posts)
            .await
    ...
    pub async fn list_draft_posts(&self) -> Result<Vec<BlogPost>> {
        ...
        let folder_result = self
            .dropbox_client
            .list_folder(&self.folders.drafts)
            .await
    ...
    async fn load_blog_post_from_file(
        ...
        let content = self
            .dropbox_client
            .download_text_file(&file_metadata.path_display)
            .await
    ...
    fn parse_frontmatter(
        ...
        let created_at = yaml_map
            .get(&serde_yaml::Value::String("created_at".to_string()))
            .and_then(|v| v.as_str())
            .and_then(|s| DateTime::parse_from_rfc3339(s).ok())
            .map(|dt| dt.with_timezone(&Utc))
            .unwrap_or_else(Utc::now);

        let updated_at = yaml_map
            .get(&serde_yaml::Value::String("updated_at".to_string()))
            .and_then(|v| v.as_str())
            .and_then(|s| DateTime::parse_from_rfc3339(s).ok())
            .map(|dt| dt.with_timezone(&Utc))
            .unwrap_or(created_at);

        let category = yaml_map
        ...
        let excerpt = yaml_map
            .get(&serde_yaml::Value::String("excerpt".to_string()))
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
...
mod tests {
    ...
    fn test_generate_slug() {
        ...
        assert_eq!(
            service.generate_slug("Special Characters @#$%"),
            "special-characters"
        );
        assert_eq!(
            service.generate_slug("Multiple   Spaces"),
            "multiple-spaces"
        );
    }
...
This is the content."#;
        ...
        let (frontmatter, body) = service
            .extract_frontmatter(content_with_frontmatter)
            .unwrap();
        ...
        let (frontmatter, body) = service
            .extract_frontmatter(content_without_frontmatter)
            .unwrap();
