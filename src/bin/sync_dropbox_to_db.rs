...
async fn main() -> Result<()> {
    ...
    let existing_slugs: std::collections::HashSet<String> =
        db_posts.into_iter().map(|post| post.slug).collect();
    ...
    for dropbox_post in dropbox_posts {
        ...
        if existing_slugs.contains(slug) {
            info!(
                "⏭️  Skipping '{}' - already exists in database",
                dropbox_post.metadata.title
            );
            skipped_count += 1;
            continue;
        }
        ...
        let html_content = match markdown_service.markdown_to_html(&dropbox_post.content) {
            ...
            Err(e) => {
                warn!(
                    "Failed to convert markdown to HTML for '{}': {}",
                    dropbox_post.metadata.title, e
                );
